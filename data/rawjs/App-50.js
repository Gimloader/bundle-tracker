...
`,D=a.div.attrs({className:"flex vc"})`
  margin-top: 17px;
`,H=e=>r.jsx(V,{onClick:e.select,style:{background:e.selected?"#1565c0":"rgba(255, 255, 255, 0.2)"},children:e.name}),J=e=>{const{gui:t}=j(),o=i=>t.selectedPlayerId=i;return r.jsx(Q,{children:e.players.map(i=>r.jsx(H,{name:i.name,selected:i.selected,select:()=>o(i.id)},`player-item-${i.id}`))})},Q=a.div.attrs({className:"flex wrap scroll-y"})`
  padding: 20px 35px;
`,V=a.div`
  padding: 5px 12px;
  border-radius: 4px;
...
`,l={out:{background:"rgba(0,0,0,0)"},in:{background:"rgba(0,0,0,0.6)"}},p={out:{y:"100%",opacity:0},in:{y:0,opacity:1}},ft=C(()=>{const{gui:e}=j(),t=W(),o=()=>{e.selectedPlayerId="",A()&&F()?g(y.startScreen):g(y.home)};K(w.Input.Keyboard.KeyCodes.ESC,o,[o]);const[i,u]=n.useState(m());n.useEffect(()=>{const s=f(()=>e.playersManagerUpdateCounter,()=>{u(m())},{fireImmediately:!0}),b=f(()=>e.selectedPlayerId,()=>{u(m())},{fireImmediately:!0});return()=>{s(),b()}},[]);const c=n.useMemo(()=>e.selectedPlayerId?i.find(s=>s.id===e.selectedPlayerId):null,[e.selectedPlayerId,i.length]);n.useEffect(()=>{!c&&i.length&&(e.selectedPlayerId=i[0].id)},[c,i.length]);const x=n.useMemo(()=>t?.3:.2,[t]),v=()=>i.length?c?r.jsxs(r.Fragment,{children:[r.jsx(R,{selectedPlayer:c,close:o}),r.jsx(J,{players:i})]}):null:r.jsx("div",{className:"flex-center maxAll",style:{padding:10,fontSize:18,fontWeight:P.Bold},children:"No other players have joined this game."});return r.jsxs(X,{children:[r.jsx(Y,{initial:l.out,animate:l.in,exit:l.out,transition:{duration:x,ease:"easeOut"},onClick:o,children:r.jsx(Z,{initial:p.out,animate:p.in,exit:p.out,transition:{duration:x,ease:"easeOut"},onClick:s=>s.stopPropagation(),children:v()})}),r.jsx(T,{onClick:o})]})}),X=a(N).attrs({className:"maxAll"})``,Y=a(k.div).attrs({className:"maxAll flex-center"})`
  background: rgba(0, 0, 0, 0.6);
  overflow: hidden;
`,Z=a(k.div).attrs({className:"medium-shadow flex-column"})`
  position: relative;
