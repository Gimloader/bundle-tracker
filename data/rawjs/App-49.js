...
`,Z=t=>{const{session:{customTeams:s}}=M(),o=h(),a=[];o.forEach(n=>{const c=s.characterToTeamMap.get(n.id);c&&c!==t||a.push({id:n.id,name:n.name,selected:c===t})});const m=a.sort((n,c)=>n.name.toLowerCase().localeCompare(c.name.toLowerCase())),T=m.filter(n=>n.selected),C=m.filter(n=>!n.selected);return T.concat(C)},tt=(t,s)=>{s?r.session.customTeams.characterToTeamMap.has(t)?r.session.customTeams.characterToTeamMap.delete(t):r.session.customTeams.characterToTeamMap.set(t,s):r.session.customTeams.characterToTeamMap.delete(t)},et=t=>{h().forEach(o=>{r.session.customTeams.characterToTeamMap.has(o.id)||r.session.customTeams.characterToTeamMap.set(o.id,t)})},st=G(t=>{const s=Z(t.id),o=s.filter(a=>a.selected).length;return e.jsxs(ot,{children:[e.jsxs(at,{children:[e.jsx(rt,{children:t.name}),e.jsx("div",{children:o!==s.length?e.jsx(W,{style:{color:U.Yellow,fontFamily:l.ProductSans,fontStyle:"italic",textDecoration:"underline"},onClick:()=>{et(t.id)},children:"Assign all remaining players"}):null})]}),e.jsx(u,{wrap:!0,direction:"horizontal",style:{marginTop:22},children:s.map(a=>e.jsx(Q,{name:a.name,selected:a.selected,onSelect:()=>{tt(a.id,t.id)}},t.id+a.id))})]})}),ot=i.div.attrs({className:"maxWidth light-shadow"})`
...
`,it=()=>{const t=x();return e.jsxs(nt,{children:[e.jsx(q,{}),e.jsx(j,{}),e.jsx(u,{className:"maxWidth",direction:"vertical",size:15,children:t.map(s=>e.jsx(st,{id:s.id,name:s.name},s.id))})]})},nt=i.div.attrs({className:"maxAll scroll-y"})`
  padding: 35px;
`,ve=()=>{const t=B(),s=()=>{F(E.startScreen)},o=S.useMemo(()=>t?.3:.2,[t]);return v($.Input.Keyboard.KeyCodes.ESC,s,[s]),w(),e.jsxs(mt,{children:[e.jsx(N,{initial:p.out,animate:p.in,exit:p.out,transition:{duration:o,ease:"easeOut"},onClick:s,children:e.jsx(ct,{initial:d.out,animate:d.in,exit:d.out,transition:{duration:o,ease:"easeOut"},onClick:a=>a.stopPropagation(),children:e.jsx(it,{})})}),e.jsx(A,{onClick:s})]})},mt=i(z).attrs({className:"maxAll"})``,ct=i(O)`
  max-width: 1000px;
  height: 90%;
