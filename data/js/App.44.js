...
c.register('.....', function (d, e) {
    ...
    var _r = () => {
        const [s, t] = h.useState(''), [u, v] = h.useState(''), [w, x] = h.useState(null), [y, z] = h.useState(!1), A = !(!s || !u || y), B = () => {
                if (!A)
                    return;
                if (s.length < 6)
                    return x('Password must contain at least 6 characters');
                if (s !== u)
                    return x('Passwords do not match');
                const C = (0, o.getUrlVariable)('token');
                if (!C)
                    return x('Invalid password reset link');
                z(!0), (0, o.request)({
                    url: '/api/user/reset',
                    method: 'post',
                    data: {
                        newPassword: s,
                        token: decodeURIComponent(C)
                    },
                    success: () => {
                        (0, o.request)({
                            url: '/logout',
                    ...
                    error: D => {
                        D && D.message && D.message.text && 'Invalid token' === D.message.text ? x('This password reset link has expired.') : x(D);
                    },
                    both: () => z(!1)
                });
            };
        return (0, g.jsxs)(g.Fragment, {
            children: [
                (0, g.jsx)(o.Title, { title: 'New Gimkit Password' }),
                (0, g.jsxs)('div', {
