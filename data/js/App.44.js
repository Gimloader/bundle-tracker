...
a.register('RGLsT', function(b, c) {
    ...
    var e = a('0hzx+'),
        ...
        o = a('RJjEi');
    var _p = () => {
        const [_q, r] = f.useState(''), [s, t] = f.useState(''), [u, v] = f.useState(null), [w, x] = f.useState(!1), y = !(!_q || !s || w), z = () => {
            if (!y)
                return;
            if (_q.length < 6)
                return v('Password must contain at least 6 characters');
            if (_q !== s)
                return v('Passwords do not match');
            const A = (0, m.getUrlVariable)('token');
            if (!A)
                return v('Invalid password reset link');
            x(!0), (0, m.request)({
                url: '/api/user/reset',
                method: 'post',
                data: {
                ...
                error: _q => {
                    _q && _q.message && _q.message.text && 'Invalid token' === _q.message.text ? v('This password reset link has expired.') : v(_q);
                },
                both: () => x(!1)
            });
        };
    ...
    a('LEQ5w');
    var e = a('NIZyA');
    var _f = _h => _h.error ? (0, d.jsx)(e.default, {
        message: 'Error',
        style: _h.style,
        type: 'error',
        description: _h.error && _h.error.message && _h.error.message.text ? _h.error.message.text : 'string' == typeof _h.error ? _h.error : 'Unknown Error. Please try again.',
        showIcon: !0
    }) : null;
