...
c.register('KcvMM', function(b, t) {
    ...
    var e = c('8kSQZ'),
        ...
        o = c('1//3g');
    var p = () => {
        const [q, r] = f.useState(''), [s, t] = f.useState(''), [u, v] = f.useState(null), [w, x] = f.useState(!1), y = !(!q || !s || w), z = () => {
            if (!y)
                return;
            if (q.length < 6)
            ...
            x(!0), (0, m.request)({
                ...
                error: q => {
                    q && q.message && q.message.text && 'Invalid token' === q.message.text ? v('This password reset link has expired.') : v(q);
                },
                both: () => x(!1)
            });
        };
