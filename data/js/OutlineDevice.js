...
b.register('.....', function(c, d) {
    ...
    class _i extends g.default {
        constructor(j) {
            super(j), (0, f.default)(this, 'onStateChange', k => {}), (0, h.CreateVisuals)(this);
        }
    }
    var _j = _i;
}), b.register('.....', function(c, d) {
    ...
    var e = b('.....'),
        ...
        j = b('.....');
    const _k = l => {
        const m = l.options.shape === e.OutlineShape.line,
            n = l.options.shape === e.OutlineShape.circle,
            o = l.options.style === e.OutlineStyle.solid,
            p = l.options.style === e.OutlineStyle.dashed;
    ...
    var e = b('.....');
    const _f = g => {
        const h = 2 * g.options.circleRadius + g.options.thickness / 2,
            i = 2 * g.options.circleRadius + g.options.thickness / 2,
            j = -Math.PI / 2 - g.options.circlePercentage / 100 * Math.PI,
            k = -Math.PI / 2 + g.options.circlePercentage / 100 * Math.PI;
        ...
        const l = g.options.circlePercentage / 100 * Math.PI * 2,
            m = Math.PI * g.options.circleRadius * 2,
            n = m * g.options.circlePercentage / 100,
            o = g.options.dashLength,
            p = g.options.dashSpacing,
            q = o / m * Math.PI * 2,
            r = p / m * Math.PI * 2;
        if (r >= n) {
            const s = Math.min(j, -q / 2),
                t = Math.max(k, q / 2);
            return void g.graphics.view.arc(0, 0, g.options.circleRadius, s, t, !1, 0.01);
        }
        const u = q + r,
            v = Math.max(1, Math.floor(l / u)),
            w = l - v * u,
            x = Math.min(q, (w + q) / 2),
            y = Math.max(0, (w - q) / 2);
    ...
    const _e = (f, g, h, i, j) => {
        ...
        const k = -Math.PI / 2 - f.options.circlePercentage / 100 * Math.PI,
            l = -Math.PI / 2 + f.options.circlePercentage / 100 * Math.PI,
            m = Phaser.Math.Angle.Between(0, 0, g, h);
        if (m < k || m > l)
            return !1;
    ...
    var e = b('.....');
    const _f = g => {
        const h = g.options.lineLength,
            i = g.options.thickness;
        g.graphics = g.parts.add.graphics({
            x: 0,
            y: 0,
            width: h,
        ...
        const j = g.options.dashLength,
            k = g.options.dashSpacing;
        if (k >= h) {
            const l = Math.min(j, h);
            return void g.graphics.view.fillRect(-l / 2, -i / 2, l, i);
        }
        const m = j + k,
            n = Math.max(1, Math.floor(h / m)),
            o = h - n * m,
            p = Math.min(j, (o + j) / 2),
            q = Math.max(0, (o - j) / 2);
    ...
    const _f = g => {
        const h = g.x,
            i = g.y,
            j = (0, e.ConvertDegToRad)(g.options.angle),
            k = g.options.lineLength / 2,
            l = h - k * Math.cos(j),
            m = i - k * Math.sin(j),
            n = h + k * Math.cos(j),
            o = i + k * Math.sin(j),
