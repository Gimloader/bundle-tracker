('undefined' != typeof globalThis ? globalThis : 'undefined' != typeof self ? self : 'undefined' != typeof window ? window : 'undefined' != typeof global ? global : {}).parcelRequire388b.register('cOqKD', function(a, b) {
    ...
    a.exports, c = function a() {
            ...
            i = {
                ...
                unparse: function(j, k) {
                    ...
                    function _y(z, A, B) {
                        ...
                        var D = Array.isArray(z) && 0 < z.length,
                            E = !Array.isArray(A[0]);
                        if (D && m) {
                        ...
                        for (var G = 0; G < A.length; G++) {
                            var H = D ? z.length : A[G].length,
                                I = !1,
                                J = D ? 0 === Object.keys(A[G]).length : 0 === A[G].length;
                    ...
                    function _M(N, O) {
                        ...
                        var P = N.toString().replace(I, E),
                            Q = 'boolean' == typeof y && y || 'function' == typeof y && y(N, O) || Array.isArray(y) && y[O] || function(R, S) {
                ...
                function n() {
                    if (0 !== m.length) {
                        ...
                        if (_ob(k.before)) {
                            ...
                            if ('object' == typeof s) {
                                if ('abort' === s.action)
                                    return o = r.file, p = r.inputElem, n = s.reason, void(_ob(k.error) && k.error({
                                        name: 'AbortError'
                                    }, o, p, n));
                                if ('skip' === s.action)
                                    return void h();
        ...
        function _x(y) {
            var z, A, B, C = Math.pow(2, 53),
                D = -C,
                E = /^\s*-?(\d+\.?|\.\d+|\d+\.\d+)(e[-+]?\d+)?\s*$/,
                F = /(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))/,
                G = this,
                H = 0,
                I = 0,
                J = !1,
            ...
            this.parse = function(W, X, Y) {
                ...
                if (y.newline || (y.newline = function($, ab) {
                        $ = $.substring(0, 1048576);
                        var bb = new RegExp(_Q(ab) + '([^]*?)' + _Q(ab), 'gm'),
                            cb = ($ = $.replace(bb, '')).split('\r'),
                            db = $.split('\n'),
                ...
                var ab = _kb(y);
                return y.preview && y.header && ab.preview++, Q = W, A = new _R(ab), M = A.parse(Q, X, Y), _P(), _t ? {
                    meta: {
                        paused: !0
                    }
                } : M || {
        ...
        function _R(S) {
            var T, U = (S = S || {}).delimiter,
                V = S.newline,
                W = S.comments,
                X = S.step,
                Y = S.preview,
                Z = S.fastMode,
                $ = T = void 0 === S.quoteChar ? '"' : S.quoteChar;
